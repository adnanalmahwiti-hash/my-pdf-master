...
def process_reducer(uploaded_files, deep=True):
    ...
    for idx, f in enumerate(uploaded_files):
        ...
        results.append({"name": f"opt_{f.name}", "data": data})
...
def process_universal_merger(uploaded_files, password=None, rotations={}):
    ...
    for idx, f in enumerate(uploaded_files):
        ...
        if ext == "pdf":
            ...
            img_io = BytesIO(); img.save(img_io, format='JPEG', quality=85)
...
if app_mode == "🔄 Converter Mode":
    ...
    if tool == "Universal Merger":
        ...
        with col_clr:
            st.write("###")
            if st.button("🗑️ Reset", help="Hard reset for cookies/multi-select fix"): hard_reset()
        ...
        if ico_files and st.button("CONVERT TO ICO"):
            res = process_ico_maker(ico_files)
            for r in res: st.download_button(f"📥 Download {r['name']}", data=r['data'], file_name=r['name'])
...
# --- REDUCER MODE ---
    ...
    if red_files:
        mode = st.radio("Compression", ["Standard", "Deep Squeeze"])
        if st.button("START REDUCTION"):
            ...
            write_global_log(mode, len(red_files), s_in, s_out, 1.0)
